...
export async function createContractProfile(
  _prev: ContractProfileFormState,
  formData: FormData
): Promise<ContractProfileFormState> {
  ...
  redirect(`/contratos/${profileId}`);
}

export type RenameContractProfileState = { error?: string };

export async function renameContractProfile(
  profileId: string,
  _prev: RenameContractProfileState,
  formData: FormData
): Promise<RenameContractProfileState> {
  try {
    const user = await assertCanWrite();
    const name = String(formData.get("name") ?? "").trim();
    if (!name) throw new Error("Indique o nome do contrato.");
    ...
    return { error: e instanceof Error ? e.message : "Não foi possível renomear o contrato." };
  }
}
...
export async function setContractProfileActive(
  profileId: string,
  active: boolean
  ...
  try {
    ...
    return { error: e instanceof Error ? e.message : "Não foi possível alterar o estado do contrato." };
  }
}
...
export type AssignContractFormState = { error?: string };

export async function assignEmployeeContract(
  _prev: AssignContractFormState,
  formData: FormData
): Promise<AssignContractFormState> {
  ...
  try {
    ...
    return { error: e instanceof Error ? e.message : "Não foi possível atribuir o contrato." };
  }
...
export type ContractTypeFormState = { error?: string };

// Tipos de contrato configuráveis (tal como os tipos de ausência).
export async function createContractType(
  _prev: ContractTypeFormState,
  formData: FormData
): Promise<ContractTypeFormState> {
  try {
    const user = await assertCanWrite();
    ...
    return { error: e instanceof Error ? e.message : "Não foi possível criar o tipo de contrato." };
  }
}

export async function deleteContractType(contractTypeId: string): Promise<{ error?: string }> {
  try {
    const user = await assertCanWrite();
    ...
    return { error: e instanceof Error ? e.message : "Não foi possível remover o tipo de contrato." };
  }
}
