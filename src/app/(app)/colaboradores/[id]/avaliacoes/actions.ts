...
export async function scheduleEvaluation(
  ...
  templateId: string,
  scheduledDate: string
): Promise<ScheduleEvaluationState> {
  try {
    ...
    return { error: e instanceof Error ? e.message : "Ocorreu um erro ao agendar a avaliação." };
  }
}
...
export type SubmitEvaluationAnswersState = { error?: string; employeeId?: string };

export async function submitEvaluationAnswers(
  evaluationId: string,
  respondent: Respondent,
  answers: AnswerInput[]
): Promise<SubmitEvaluationAnswersState> {
  try {
    const user = await requireUser();
    ...
    return { error: e instanceof Error ? e.message : "Ocorreu um erro ao gravar as respostas." };
  }
}
